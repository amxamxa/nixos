{ config, lib, pkgs, modulesPath, ... }: {
  ...
  fileSystems."/docker" = {
    device =
      "/dev/disk/by-label/docker"; # /dev/sdb2: LABEL="docker" UUID="81c7ce53-592b-44a4-aa8a-a4add588e292"
    fsType = "ext4";
    options = [
      "defaults" # "subvol=share" "noatime" "nodiratime" "discard"
    ];
  };
  ...
  virtualisation.docker = {
    ...
    enableOnBoot =
      false; # When enabled dockerd is started on boot. required for containers with --restart=always' flag.
    # If disabled, docker might be started on demand by socket activation.
    rootless = {
      enable =
        true; # Podman socket avail Docker ohne Root-Rechte betreiben (sicherer)
    ...
    daemon.settings = {
      ...
      data-root =
        "/home/amxamxa/.local/share/docker"; # Beispielhafter Pfad im Home-Verzeichnis
