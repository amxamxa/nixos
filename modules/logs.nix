...

{
  ...
  services.journald = {
    ...
    extraConfig = ''
      ...
      # Keep max 8 journal files (8 × 64M = 512M)
      SystemMaxFiles=8
      # Maximum RAM for volatile logs
      RuntimeMaxUse=128M
      # Maximum size per runtime journal file
      ...
      Compress=yes
      # Cryptographically seal logs (tamper detection)
      Seal=yes
      # Separate logs per user (better security)
      SplitMode=uid
      ...
      # -─────────────────────────────
      ForwardToSyslog=no
      # Don't duplicate to rsyslog (we use journald only)
      # Forward to kernel ring buffer
      ForwardToKMsg=yes
      # Do spam console with logs
      ForwardToConsole=yes
      # Don't broadcast critical messages to all terminals
      ForwardToWall=no
      ...
      # -─────────────────────────────
      # Store info and higher (info, notice, warning, err, crit, alert, emerg)
      MaxLevelStore=info
      # Forward info and higher to syslog
      MaxLevelSyslog=info
      # Forward notice and higher to kernel buffer
      MaxLevelKMsg=notice
      # Only show warnings+ on console
      MaxLevelConsole=warning
      # Only broadcast emergencies
      MaxLevelWall=emerg
    '';
  };
  ...
  services.logrotate = {
    ...
    configFile = pkgs.writeText "logrotate.conf" ''
      ...
      /var/log/nixos/sudo.log {
        weekly
        # Keep 3 months of sudo logs
        rotate 12
        compress
        missingok
        notifempty
        # Restrictive permissions for security logs
