...
#  	fsIdentifier = "label"; # test: findfs LABEL=label
 	devices = [ "nodev" ];
  	useOSProber = true;
  	default = "saved"; #index of the default menu item to be booted. Can also be set to “saved”, which will make GRUB select the menu item that was used at the last boot.
...
# Enable the XFCE Desktop Environment.
 services.xserver.desktopManager = {
	...
	wallpaper.combineScreens= true;	}; # wallpaper will stretch across all screens
...
#--------------
 services.journald.extraConfig = ''
	    		...
	    		'';	
	 #----------------
  services.picom = {  # service for xfce display mgmt
       enable = true;
       fade = true;
...
#	round-borders-rule = [ #syntax format `PIXELS:PATTERN`
#	  	  "10:class_g      = 'obsidian'"
#	  	  "3:class_g      = 'kitty'"        ];
...
#		  "class_g = 'Conky'&& window_type = 'desktop'"
		   # prevents blurring the background when taking screenshot
#		    "class_g = 'slop'"
#		  "_GTK_FRAME_EXTENTS@:c" ];  
...
##	refresh-rate = 0; # If not specified or 0, picom will try detecting this with X RandR extension.
