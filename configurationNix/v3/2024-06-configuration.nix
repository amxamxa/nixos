...
#	      (nixpkgs.fetchFromGitHub {
   ...
   gnome.gdm
		...
		cinnamon.nemo-with-extensions  	#cinnamon.nemo 	cinnamon.nemo-emblems # change a folder or file emblem # namon.nemo-fileroller # compress and extract functions of the FileRoller archive adds entries to the context menu in Nemo # cinnamon.nemo-python 	 cinnamon.folder-color-switcher #Change folder colors for Nemo and Caja
    #			------------------------------------
    ### 		XFCE tools
    #			------------------------------------
  ...
  #backgroundColor  = "#7EBAE4"; # background color to be used to fill the areas the image isn’t filling.
	...
	# extraConfig = ''
 	#		set color_normal = red
  	...
  	# 				'';
  	extraEntries = ''
      menuentry "netboot.xyz" {  #tba
    	}  
        			'';
  
 # extraEntries = ''
 #      menuentry "parrot" {
 #        search --set=parrot --fs-uuid 4589e5aa-b425-4095-bd45-c39426bc2a1d
...
# Sudoers - I/O plugin version 1.9.15p2  Sudoers - audit plugin version 1.9.15p2
	security.sudo = {
	  extraRules =   [
	    { groups =   ["wheel"];
	      commands = [
	        {	command = "${pkgs.coreutils}/sbin/df";
	        	options = ["NOPASSWD"];      	} 
